            ...
            <tr v-for="option in items" :key="option.id">
              ...
              <td class="action-btnlist col-actions">
                <template v-if="editingId === option.id">
                  ...
                  <button class="btn btn-light btn-sm" :disabled="savingId === option.id" @click="cancelEdit">
                    Cancel
                  </button>
                </template>
                <template v-else>
...
async function create() {
  ...
  creating.value = true;
  try {
    const sortOrder = items.value.length ? Math.max(...items.value.map((o) => o.sort_order)) + 1 : 0;
...
async function remove(option) {
  ...
  if (props.usageColumn) {
    ...
    if (countErr) {
      alert("Failed to check usage: " + countErr.message);
      return;
    }
    if (count) {
