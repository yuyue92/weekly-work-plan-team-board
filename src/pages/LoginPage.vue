  ...
  <div class="auth-wrap">
    <div class="auth-card">
      ...
      <template v-if="isResetRoute">
        <div class="form-group">
          ...
          <div class="auth-password-wrap">
            <input
              ...
              v-model="password"
              @keyup.enter="doUpdatePassword"
            />
            <button type="button" class="auth-password-toggle" @click="showPassword = !showPassword">
              <svg v-if="showPassword" viewBox="0 0 24 24" aria-hidden="true">
                <path
                  d="M3 3l18 18M10.6 10.7a2 2 0 002.7 2.7M9.9 4.2A10.6 10.6 0 0112 4c5.5 0 9 5 9 5a16 16 0 01-3.1 3.6M6.2 6.2C4.2 7.5 3 9 3 9s3.5 5 9 5a10.8 10.8 0 004-.8"
                />
              </svg>
        ...
        <p class="auth-hint">
          After saving, this temporary session will end and you'll need to log in with your new password.
        </p>
      </template>
      ...
      <template v-else-if="mode === 'forgot'">
        <div class="form-group">
          ...
          <input
            ...
            type="email"
            v-model="email"
            placeholder="xxx@pccw.com"
            @keyup.enter="doRequestReset"
          />
        </div>
        <div v-if="errorMsg" class="auth-error">{{ errorMsg }}</div>
      ...
      <template v-else>
        ...
        <!-- 登录表单 -->
        <div v-if="mode === 'login'">
          <div v-if="resetSuccessNotice" class="auth-success">
...
function friendlyAuthError(error) {
  ...
  if (
    message.includes("uq_profiles_staff_id") ||
    message.includes("duplicate key") ||
    message.includes("staff id already exists")
  ) {
  ...
  if (message.includes("invalid login credentials"))
    return "Incorrect email or password, or this email hasn't registered yet.";
  if (message.includes("email not confirmed"))
    return "This email hasn't been verified yet. Please check your inbox for the verification link.";
...
async function doRegister() {
  ...
  if (!displayName.value.trim()) {
    errorMsg.value = "Please enter a display name";
    return;
  }
  if (!email.value || !password.value) {
    errorMsg.value = "Please enter your email and password";
  ...
  localStorage.setItem(REMEMBER_EMAIL_KEY, normalizedEmail);
  successMsg.value =
    "Sign-up successful! Please check your email to verify your account, then log in from the [Log In] tab.";
  staffId.value = "";
  password.value = "";
  mode.value = "login";
}
...
async function doRequestReset() {
  if (!email.value) {
    errorMsg.value = "Please enter your registered email";
    return;
  }
  submitting.value = true;
  clearMsg();
...
async function doUpdatePassword() {
  ...
  if (password.value !== confirmPassword.value) {
    errorMsg.value = "The two passwords you entered don't match";
    return;
  }
